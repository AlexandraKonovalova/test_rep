...
def rel(eng, cham):
    sent_eng = [len(line.split()) for line in eng.split('\n') if len(line) > 0]
    sent_cham = [len(line.split()) for line in cham.split('\n') if len(line) > 0]
    for i, line in enumerate(sent_eng):
        if i <= 100:
            comp = round((line / sent_cham[i]), 1)
            print('Предложение в английском длиннее в', comp, 'раз(а)')
